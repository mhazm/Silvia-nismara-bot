const { ChatInputCommandInteraction, ApplicationCommandOptionType, AttachmentBuilder, EmbedBuilder, ActionRowBuilder, StringSelectMenuBuilder } = require("discord.js");
...
module.exports = new ApplicationCommand({
    ...
    run: async (client, interaction) => {
        ...
        try {
            const active = await ActiveJob.findOne({ guildId, driverId: userId, active: true });
            if (!active) return interaction.editReply("❌ Kamu tidak punya job aktif.");

            // Ambil data job terbaru
            const res = await fetch(`https://e.truckyapp.com/api/v1/job/${active.jobId}`, {
                headers: {
            ...
            if (job.status !== "completed") {
                return interaction.editReply("⚠️ Job ini belum selesai di Trucky! Pastikan sudah complete sebelum end.");
            }
            ...
            .setColor("Green")
            .setAuthor({ name: job.driver.name, iconURL: job.driver.avatar_url })
            .addFields(
                ...
                { name: "🚚 Rute", value: `${job.source_city_name} → ${job.destination_city_name} (${job.real_driven_distance_km} km)` },
            ...
            const finesTotalFromList = Array.isArray(finesArr) && finesArr.length > 0
            ...
            let economyReport = [];
            economyReport.push(`🏦 Pendapatan Kotor  : ${Number(job.income || 0).toLocaleString()} T¢`);
            economyReport.push(`💵 Pendapatan Bersih : ${Number(job.revenue || 0).toLocaleString()} T¢`);
            economyReport.push(`💸 Pajak             : ${taxes.toLocaleString()} T¢`);
            economyReport.push(`🧾 Biaya Sewa        : ${rentCost.toLocaleString()} T¢`);
            economyReport.push(`⛽ Bahan Bakar       : ${Number(job.fuel_used_l || 0).toFixed(1)} L (${(Number(job.fuel_cost || 0)).toFixed(2)} T¢)`);
            economyReport.push(`⚙️ Efisiensi BBM     : ${job.fuel_economy_l100km ?? "-"} L/100km`);

            // section lain (only if exists)
            if (otherCosts > 0) {
                economyReport.push("");
                economyReport.push(`💼 Biaya Lain-lain   : ${otherCosts.toFixed(2)} T¢`);
            }

            if (damageCost > 0) {
            let damageDetails = {};
            try { damageDetails = JSON.parse(job.damage_cost_details || "{}"); } catch(e){ damageDetails = {}; }
                economyReport.push("");
                economyReport.push(`💥 Biaya Kerusakan   : ${damageCost.toFixed(2)} T¢`);
                economyReport.push(`  🚛 Truk    : ${damageDetails.vehicle_damage ?? 0}%`);
                economyReport.push(`  🛞 Trailer : ${damageDetails.trailers_damage ?? 0}%`);
                economyReport.push(`  📦 Kargo   : ${damageDetails.cargo_damage ?? 0}%`);
            }
            ...
            if (Array.isArray(finesArr) && finesArr.length > 0) {
                ...
                finesArr.slice(0, showLimit).forEach(f => {
                    ...
                    economyReport.push(`• ${label} — ${Number(f.amount || 0).toLocaleString()} T¢`);
                });
                
                if (finesArr.length > showLimit) {
                    economyReport.push(`(+${finesArr.length - showLimit} denda lainnya)`);
                }
            }
            ...
            page2.setDescription("```yaml\n" + economyReport.join("\n") + "\n```")
                .setTimestamp()
                .setFooter({ text: "Halaman 2 • Laporan Keuangan & Statistik Ekonomi" });
            ...
            if (job.realistic_points_calculation || job.realistic_ldb_points) {
                ...
                .setColor("Blue")
                .addFields(
                    { name: "🏁 Total Poin", value: `${job.realistic_ldb_points ?? 0}`, inline: true },
                    { name: "⭐ Rating Realistis", value: `${job.delivery_rating_details?.rating ?? 0}`, inline: true },
                    { name: "📏 Jarak", value: `${rp.distance?.toFixed(1) ?? 0}`, inline: true },
                    { name: "⚖️ Berat Muatan", value: `${rp.mass?.toFixed(1) ?? 0}`, inline: true },
                    { name: "🚫 Denda", value: `${rp.fines?.toFixed(1) ?? 0}`, inline: true },
                    { name: "💥 Kerusakan", value: `${rp.damage?.toFixed(1) ?? 0}`, inline: true },
                    { name: "⛽ Efisiensi BBM", value: `${rp.fuel_economy?.toFixed(1) ?? 0}`, inline: true },
                    { name: "🅿️ Parkir Ganda", value: `${rp.hard_parking_doubles?.toFixed(1) ?? 0}`, inline: true },
                )
                .setTimestamp()
            ...
            if (!notifyChannel) {
                return interaction.editReply("❌ Channel kontrak tidak ditemukan atau belum diatur.");
            }
            ...
            const allowedRoles = ["1077368181926141973", "1405532668472590437", "1333622587749564619"]; // Manager, Moderator, dsb

            // 🔹 Collector aktif sampai 1 hari (24 jam = 86400000 ms)
            const collector = message.createMessageComponentCollector({ time: 36000000 });
            ...
            console.error(err);
            await interaction.editReply("⚠️ Terjadi kesalahan saat menyelesaikan job.");
        }
    }
}).toJSON();