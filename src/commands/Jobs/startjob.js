	...
	run: async (client, interaction) => {
		...
		try {
			...
			if (job.status === 'completed') {
				return interaction.editReply(
					'❌ Job ini sudah selesai, tidak bisa digunakan untuk kontrak aktif.',
				);
			}
			...
			const existing = await ActiveJob.findOne({
				guildId,
				driverId: userId,
				active: true,
			});
			if (existing)
				return interaction.editReply(
					'⚠️ Kamu masih punya job aktif! Selesaikan dulu job sebelumnya dengan `/endjob`.',
				);
			...
			await interaction.editReply(
				`✅ Job **${job.cargo_name}** dari **${source}** dimulai! Selesaikan dan akhiri dengan perintah \`/endjob\`.`,
			);
		} catch (err) {
			console.error(err);
			await interaction.editReply(
				'⚠️ Terjadi kesalahan saat memulai job.',
			);
		}
	},
}).toJSON();
