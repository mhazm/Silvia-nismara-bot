...
module.exports = new ApplicationCommand({
    ...
    run: async (client, interaction) => {
        ...
        try {
            ...
            const history = await PointHistory.find({ guildId, userId }).sort({ createdAt: -1 }).limit(10);
            const total = await Point.findOne({ guildId, userId });

            if (!history.length) return interaction.editReply("Kamu belum memiliki riwayat poin.");

            const desc = history.map(h => {
            ...
            .setColor("Blue")
            .setDescription(desc)
            .addFields({ name: "Total Poin Saat Ini", value: `${total?.totalPoints ?? 0}`, inline: false })
            .setTimestamp();
            ...
            return interaction.editReply("⚠️ Terjadi kesalahan saat memuat data dari database.");
        }
    }
}).toJSON();