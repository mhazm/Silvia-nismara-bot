		...
		try {
			...
			if (!guildId) {
				return interaction.editReply(
					'⚠️ Perintah ini hanya dapat digunakan di dalam server.',
				);
			}

			const guildAvatar = interaction.guild.iconURL({ dynamic: true });

			const history = await ContractHistory.find({ guildId: guildId })
				.sort({ startDate: -1 })
				.limit(10);

			if (!history.length) {
				return interaction.editReply(
					'📭 Belum ada data kontrak yang tersimpan.',
				);
			}
			...
			return interaction.editReply(
				'⚠️ Terjadi kesalahan saat memuat riwayat kontrak.',
			);
		}
	},
}).toJSON();
