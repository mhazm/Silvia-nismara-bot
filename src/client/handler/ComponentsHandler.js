				...
				try {
					...
					if (module.__type__ === 3) {
						if (!module.customId || !module.type || !module.run) {
							error(
								'Unable to load the button/select/modal component ' +
									file,
							);
							continue;
						}
						...
						info(
							`Loaded new component (type: ${module.type}) : ` +
								file,
						);
					} else if (module.__type__ === 4) {
						if (!module.commandName || !module.run) {
							error(
								'Unable to load the autocomplete component ' +
									file,
							);
							continue;
						}
						...
						info(
							`Loaded new component (type: autocomplete) : ` +
								file,
						);
					} else {
						error(
