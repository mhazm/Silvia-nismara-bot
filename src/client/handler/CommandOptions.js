...
const handleApplicationCommandOptions = async (interaction, options, command) => {
    ...
    // 🔹 ALLOWED ROLES (dinamis dari DB)
    if (options.allowedRoles && Array.isArray(options.allowedRoles) && options.allowedRoles.length > 0) {
        const member = interaction.member ?? await interaction.guild.members.fetch(interaction.user.id);

        // 🔍 Ambil setting guild dari database
        const guildSettings = await GuildSettings.findOne({ guildId: interaction.guild.id });

        // 🔁 Gabungkan role ID dari DB dan config (fallback)
        const resolvedRoles = options.allowedRoles.flatMap(roleKey => {
            const dbRoles = guildSettings?.roles?.[roleKey] ?? [];
            const cfgRoles = config.users.roles?.[roleKey] ?? [];
            return [...dbRoles, ...cfgRoles];
        });

        // 🔐 Cek apakah user punya salah satu role
        const hasRole = member.roles.cache.some(role => resolvedRoles.includes(role.id));

        if (!hasRole) {
            await interaction.reply({
                content: config.messages.MISSING_REQUIRED_ROLE || 
                        "❌ Kamu tidak memiliki role yang diizinkan untuk menjalankan command ini.",
                ephemeral: true,
            });
            return false;
        }
