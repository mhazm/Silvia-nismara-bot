...
const {
	validateAndFetchJob,
} = require('../../services/JobProcessor/JobValidator');
const {
	calculateExpenses,
	formatOffenceName,
} = require('../../services/JobProcessor/ExpenseCalculator');
const {
	calculateRewards,
} = require('../../services/JobProcessor/RewardCalculator');
const {
	calculatePenaltyAndXp,
	formatStatsType,
} = require('../../services/JobProcessor/PenaltyAndXpCalculator');
const {
	updateDatabase,
} = require('../../services/JobProcessor/DatabaseUpdater');
		...
		try {
			...
			if (settings.channelLog) {
				...
				if (logChannel) {
					...
					if (reward.nismaraplus > 0)
						ncField.push({
							name: '💜 Nismara Plus Bonus Earned',
							value: `+${reward.nismaraplus} N¢`,
							inline: true,
						});
					if (maintenancePenaltyAmount > 0)
						ncField.push({
							name: '🔧 Maintenance Penalty',
							value: `-${maintenancePenaltyAmount} N¢`,
							inline: false,
						});
					if (discount.insurance > 0)
						ncField.push({
							name: '🛡️ Insurance Discount',
							value: `-${discount.insurance} N¢`,
							inline: false,
						});
					if (discount.nismaraplus > 0)
						ncField.push({
							name: '💜 Nismara Plus Discount',
							value: `-${discount.nismaraplus} N¢`,
							inline: true,
						});
					if (discount.total > 0)
						ncField.push({
							name: '💰 Total Discount',
							value: `-${discount.total} N¢`,
							inline: true,
						});
					if (cost.rent > 0)
						ncField.push({
			...
			if (penalty.total > 0) {
				fields.push({
					...
					value:
						context.driverJob?.gameMode === 'truckersmp'
			...
			if (reward.truckersmp > 0)
				fields.push({
					name: '🌐 TruckersMP Bonus Earned',
					value: `+${reward.truckersmp} N¢`,
					inline: true,
				});
			if (reward.taxAmount > 0) {
				const taxPercent = (reward.taxRate * 100).toFixed(0); // mengubah 0.2 menjadi 20
				fields.push({
					name: `🏦 Taxes (${taxPercent}%)`,
					value: `-${reward.taxAmount} N¢`,
					inline: true,
				});
			}
			if (reward.total > 0)
			...
			if (maintenancePenaltyAmount > 0)
				fields.push({
					name: '🔧 Vehicle Maintenance Penalty',
					value: `-${maintenancePenaltyAmount} N¢`,
					inline: false,
				});
			if (penalty.vehicle > 0)
				fields.push({
					name: '🚗 Vehicle Damage',
					value: `${job.vehicle_damage}% → **${penalty.vehicle}** points`,
					inline: false,
				});
			if (penalty.trailer > 0)
				fields.push({
					name: '🚛 Trailer Damage',
					value: `${job.trailers_damage}% → **${penalty.trailer}** points`,
					inline: true,
				});
			if (penalty.cargo > 0)
				fields.push({
					name: '📦 Cargo Damage',
					value: `${job.cargo_damage}% → **${penalty.cargo}** points`,
					inline: true,
				});
			if (penalty.distance > 0)
				fields.push({
					name: '🛣️ Distance Penalty',
					value: `${job.driven_distance_km} Km → **${penalty.distance}** points`,
					inline: true,
				});
			if (penalty.speed > 0)
				fields.push({
					name: '⚡ Speed Penalty',
					value: `${formatStatsType(job.stats_type)} → **${penalty.speed}** points`,
					inline: true,
				});
			...
			const embedUser = new EmbedBuilder()
				...
				.setFooter({
					text: 'Untuk informasi lebih detail kamu bisa download Manifest Job yang sudah dikirim di atas',
				})
				.setTimestamp()
				...
				if (notifyChannel) {
					...
					const fieldPage1 = [
						...
						},
						{
							name: '📊 Tipe Statistik',
							value:
								formatStatsType(job.stats_type) ||
								readableStats,
							inline: true,
						},
						{
					...
					if (damageCost > 0) {
						...
						economyReport.push(
							...
							`  🚛 Truk    : ${dmgDetails.vehicle_damage ?? 0}T¢`,
							`  🛞 Trailer : ${dmgDetails.trailers_damage ?? 0}T¢`,
							`  📦 Kargo   : ${dmgDetails.cargo_damage ?? 0}T¢`,
						);
					}
					if (finesArr.length > 0) {
						...
						finesArr
							...
							.forEach((f) =>
								economyReport.push(
									`• ${formatOffenceName(f.offence)} — ${Number(f.amount || 0).toLocaleString()} T¢`,
								),
							);
						if (finesArr.length > 8)
							economyReport.push(
								`(+${finesArr.length - 8} denda lainnya)`,
							);
					}
					economyReport.push(
						'',
					...
					if (
						...
						const page4 = new EmbedBuilder()
							...
							.setTimestamp()
							.setFooter({
								text: 'Halaman 4 • Realistic Points',
							});
						pages.push(page4);
						options.push({
					...
					const collector =
						contractMsg.createMessageComponentCollector({
							time: 3600000,
						});
			...
			// WARNING THRESHOLD
			if (
				currentPenaltyPoints >= WARNING_THRESHOLD &&
				totalPointsBefore < WARNING_THRESHOLD
			) {
				...
				try {
					...
					if (userToDM)
						await userToDM.send({ embeds: [penaltyEmbed] });
				} catch (err) {}
				const warningChannel = message.guild.channels.cache.get(
					settings.channelLog,
				);
				if (warningChannel) {
					warningChannel.send({
						content: `⚠️ **WARNING SP1:** Driver <@${discordId}> (Trucky ID: ${truckyId}) telah mencapai limit warning poin penalti (${currentPenaltyPoints} points).`,
					});
				}
			} else if (
				currentPenaltyPoints >= WARNING_THRESHOLD &&
				currentPenaltyPoints >= totalPointsBefore + WARNING_PENALTY_ADD
			) {
				const penaltyEmbed = new EmbedBuilder()
				...
				if (warningChannel) {
					warningChannel.send({
						content: `🚨 **WARNING SP LANJUTAN:** Driver <@${discordId}> (Trucky ID: ${truckyId}) point penalti bertambah signifikan, total saat ini ${currentPenaltyPoints} points.`,
					});
				}
			}
		} catch (err) {
